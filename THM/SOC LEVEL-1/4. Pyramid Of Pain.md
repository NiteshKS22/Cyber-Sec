## Task 2: Hash Values (Trivial) — The Digital Fingerprint
...
| :--- | :--- | :--- | :--- |
| **MD5** (Message Digest 5) | 1992 | **BROKEN** 🚫 | The grandfather. It produces a 128-bit hash. It is **not** cryptographically secure anymore because of "collisions" (two different files can have the same MD5 hash). |
| **SHA-1** (Secure Hash Algorithm 1) | 1995 | **BANNED** 🚫 | The retired veteran. Created by the NSA. It produces a 160-bit hash. NIST banned it in 2013 because it is susceptible to brute-force attacks. |
| **SHA-256** (Secure Hash Algorithm 2) | 2001 | **STANDARD** ✅ | The current reliable choice. Also NSA/NIST designed. It produces a massive 256-bit hash. This is what we use today. |
...
![alt text](/THM/SOC%20LEVEL-1/Images/j4_virustotal.png)

2.  **MetaDefender Cloud (OPSWAT):** Similar to VirusTotal, excellent for cross-referencing.
![alt text](/THM/SOC%20LEVEL-1/Images/j4_metadefender.png)
...
**Conclusion: The file still works, but the hash is totally different (D1A0... vs 9D52...). This is why hunting by file hash alone is difficult—attackers can change their "fingerprints" trivially.